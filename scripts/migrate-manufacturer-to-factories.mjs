 ...
 *
 * Usage: node scripts/migrate-manufacturer-to-factories.mjs
 */
...
async function addFactoryToProduct(productId, factoryId) {
  // Twenty CRM uses "connect" to add relations in many-to-many fields
  const mutation = `
    mutation UpdateProductManufacturers($id: UUID!, $data: ProductUpdateInput!) {
...
async function migrate() {
  ...
  try {
    ...
    // Filter products that have a manufacturer (singular) but no manufacturers (plural/many-to-many)
    const productsToMigrate = products.filter(p => {
      const hasManufacturer = p.manufacturer?.id;
      const hasManufacturers = p.manufacturers?.edges && p.manufacturers.edges.length > 0;
      return hasManufacturer && !hasManufacturers;
    });
